...
def get_week_inline_markup(query: str, shift: str = 'this', with_previous: bool = False) -> InlineKeyboardMarkup:
    ...
    this_week_title = 'На цей тиждень' + ('*' if shift == 'this' else '')
    next_week_title = 'На наступний тиждень ' + ('*' if shift == 'next' else '')

    if with_previous:
        previous_week_title = 'За попередній тиждень' + ('*' if shift == 'previous' else '')
