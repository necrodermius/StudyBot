...
@bot.message_handler(regexp='^✏ Редагувати$')
...
def inline_photos_handler(call: CallbackQuery):
    ...
    if len(task.photos) < 1:
        return send_message_private(call.message, 'Немає фотографій')
...
def inline_task_handler(call: CallbackQuery):
    ...
    if option == 'cancel':
        bot.answer_callback_query(call.id, 'Відмінено')
        return save_delete_message(chat_id, message_id)

    task = get_task(id)
    if not task:
        bot.answer_callback_query(call.id, 'Завдання вже видалено', show_alert=True)
        return save_delete_message(chat_id, message_id)

    if option == 'edit':
        text = (f'Введіть змінене завдання для предмету:\n<b>{task.subject.name}</b>\n'
                f'<pre>{task.text}</pre>')
    ...
    if option == 'files':
        text = 'Відправте файл для завдання'
...
def send_task_edit_menu(message: Message, id: int, allow_editing: bool = True):
    ...
    if not task:
        return send_message_private(message, f'Завдання з id <b>{id}</b> не знайдено', reply_to_message_id=message.id)

    text = (f'<b>{task.subject.name}</b>\n'
            f'Задано на: <i>{task.date}</i>\n'
            f'Додано: <i>{task.created_at}</i>\n\n'
