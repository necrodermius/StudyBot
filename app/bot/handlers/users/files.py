...
def inline_file(inline_query: InlineQuery):
    ...
    results = [InlineQueryResultArticle(
        ...
        input_message_content=InputTextMessageContent('Файл не знайдено ❌')
    )]
    ...
    if file_available:
        ...
        results = [InlineQueryResultArticle(
            ...
            title=file.title,
            description='Схоже файл був видалений',
            thumb_url='https://images.emojiterra.com/google/android-11/512px/274c.png',
            input_message_content=InputTextMessageContent(f'Файл "<i>{file.title.lower()}</i>" не знайдено ❌',
                                                          parse_mode='HTML')
        )]
...
def inline_file_handler(call: CallbackQuery, current_user: User):
    ...
    if not file:
        return bot.answer_callback_query(call.id, 'Файл не знайдено', show_alert=True)
    ...
    try:
        ...
        if e.error_code == 400:
            return bot.answer_callback_query(call.id, 'Схоже файл був видалений', show_alert=True)
